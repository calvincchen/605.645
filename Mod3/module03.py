...
def forward_check(graph, all_colors, available_colors, vertex_colors, current_vertex, trace):
	...
	for c in available_colors[current_vertex]:
		...
		if is_safe(current_vertex, graph, vertex_colors, c):
			...
			# If there is no solution, remove color (BACKTRACK) and add back to available colors
			vertex_colors[current_vertex] = 0
...
'''
def BT(planar_map, colors, trace = False):
	#nb of vertex
	...
	if backtrack(graph, colors, vertex_colors, 0):
		return(vertex_colors)
	else:
		print("No solutions")


def FC(planar_map, colors, trace = False):
	#nb of vertex
	number_vertices = len(planar_map["nodes"])
	...
	# need to make dictionary of available colors:
	available_colors = {}
	for current_vertex in range(len(graph)):
		available_colors[current_vertex] = colors.copy()


		#beginning with vertex 0
	if forward_check(graph, colors, available_colors, vertex_colors, 0):
		return(vertex_colors)
	else:
		print("No solutions")

def MRV(planar_map, colors, trace = False):
	#nb of vertex
	number_vertices = len(planar_map["nodes"])
	# nb of colors
	#color_nb = len(colors)
	...
	for current_vertex in range(len(graph)):
		...
		#beginning with vertex 0
	if minimum_remaining_values(graph, colors, available_colors, vertex_colors, 0):
		return(vertex_colors)
	else:
		print("No solutions")
'''
...
if __name__ == "__main__":
	...
	print("Minimum Remaining Values...", "Y")
	print("Degree Heuristic...", "N")
	...
	#draw_map("connecticut", connecticut, (5,4), [("Fairfield", "red"), ("Litchfield", "blue"), ("New Haven", "red"), ("Hartford", "blue"), ("Middlesex", "red"), ("Tolland", "blue"), ("New London", "red"), ("Windham", "blue")])
	
	#find_minimum_index({1: ['blue', 'green', 'yellow'], 2: ['blue', 'green', 'yellow'], 3: ['red', 'blue', 'green', 'yellow'], 4: ['red', 'blue', 'green', 'yellow'], 5: ['red', 'blue', 'green', 'yellow'], 6: ['red', 'blue', 'green', 'yellow'], 7: ['red', 'blue', 'green', 'yellow']}, ["red", "blue", "green", "yellow"])
	#sol = BT(europe, ["red", "blue", "green", "yellow"])
	#print(sol)
	#sol = FC(europe, ["red", "blue", "green", "yellow"])
	#print(sol)
	#sol = allSolutions(europe, ["red", "blue", "green", "yellow"], 'LCV')
	#print(sol)
	#color_assignment = pretty_color_assignments(europe, sol)
	#print(color_assignment)
	#draw_map("europe", europe, (5, 4), color_assignment)
