...
def q_master(world, costs, goal, reward, actions, gamma, alpha):
	print("Q learning starting.")
	visited = {}
	# visited should take in the tuple: count
	epsilon = 1
	delta_epsilon = -.000001
	...
	visited[(goal[0] - 1, goal[1])] = 1

	# very naive and greedy implementation
	# run algorithm once from every index
	count = 0
	# Arbitrary assumption that 10000 visits to a node will end in success
	# Couldn't get Q learning to converge
	while(min(visited.values())) < 10000 and epsilon > 0:
		count +=1
		if count % 1000 == 0:
			print("Number of episodes: " + str(count))
			print("Current policy")
			display_policy = deepcopy(prev_policy)
			display_policy[goal[0]][goal[1]] = 'G'
			for i in range(len(world)):
				for j in range(len(world[0])):
					if world[i][j] == 'x':
						display_policy[i][j] = 'x'
			pretty_print_policy(0, 0, display_policy)
			print()
		# 10000 is chosen as goal b/c of of function q_episode
		# Nodes that have been visited more than 10000 times treated as terminal state
		# Starts with most visited nodes to populate table near goal
		index = find_max_visited(visited, 10000)
		#print(index)
		if index == -1:
...
def find_max_visited(visited, count):
	positions = [] # output variable
	max_value = -float("inf")
	for k, v in visited.items():
		if v == max_value:
			positions.append(k)
		if v > max_value and v < count:
			max_value = v
			positions = [] # output variable
			positions.append(k)
...
def q_episode(world, costs, goal, reward, actions, gamma, alpha, x, y, Q_right, Q_left, Q_up, Q_down, visited, epsilon):
	...
	'''if reach an index that has been visited over 10000 times, then end'''
	'''optimization. can remove?'''
	if visited[(x, y)] >= 10000 and x != goal[0] and y != goal[1]:
		print("Reached confirmed node")
...
def test_q_score():
	...
	return q_master(world, costs, goal, reward, actions, gamma, alpha)
...
def pretty_print_policy(rows, cols, policy):
	print('\n'.join([''.join(str(item) for item in row) for row in policy]))
	pass
...
if __name__ == "__main__":
	...
	print("Test World Estimated Time: 2 min")
	print("The current policy is printed every 1000 episodes.")
	goal = (5, 6)
	gamma = 0.9  # FILL ME IN
	alpha = 0.75  # FILL ME IN
	reward = 100000  # FILL ME IN
	test_policy = q_learning(test_world, costs, goal, reward, cardinal_moves, gamma, alpha)
	rows = len(test_world)  # FILL ME IN
	cols = len(test_world[0])  # FILL ME IN
	print("Final Solution")
	pretty_print_policy(rows, cols, test_policy)
	print()
	
	print("Full World Estimated Time: Unknown (> 4 hours)")
	print("The current policy is printed every 1000 episodes.")
	goal = (26, 26)
	gamma = 0.9  # FILL ME IN
	alpha = 0.75  # FILL ME IN
	...
	rows = len(full_world)  # FILL ME IN
	cols = len(full_world[0])  # FILL ME IN
	print("Final Solution")
